...
on windows text type files will convert LF -> CRLF on checkout (only) when core.autocrlf=true  
when core.autocrlf=false, then the file will get LF on checkout 
files generated by windows tools (HEX and many others) will contain CRLF and GIT will show status modified for those files  
windows clients msys2, cygwin and the main windows GIT are aware of that and convert LF -> CRLF 
